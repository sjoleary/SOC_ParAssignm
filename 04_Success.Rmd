...
Mean reproductive success at sample points T1 and T3 per breeder was determined as the mean proprotion of progeny assigned to an adult across spawning events (adults participated in one to three events). 
...
Test for significant difference among mean at at spawning using an ANOVA and pairwise comparisons using Tukey's 'Honest Significant Difference' method (based on Studentized range statistic) which uses intervals based on the range of the sample means (not individual diferences) with an adjustment for differences of observations made for each category.


**Age at spawning**

Test for heterogeneity using ANOVA and significant pairwise differences using Tukey HSD statistic.

```{r}

## test for significant diferences AGE SPAWN ----
...
p1 <- ggplot(age_spawn, aes(x = CATEGORY, y = AGE_SPAWN, fill = CATEGORY)) +
  ...
  scale_fill_manual(values = c("#4473c4", "#3d6623", "#7008A0")) +
...
# anova
anova <- aov(AGE_SPAWN~CATEGORY, data = age_spawn)

writeLines("\n RESULTS ANOVA")
summary(anova)

# Tukey
writeLines("\n \nRESULTS PAIRWISE COMPARISONS USING TUKEY HONEST SIGNIFICANT DIFFERENCES")
TukeyHSD(anova)
...
## test for significant diferences AGE INTRODUCED ----
...
# anova
anova <- aov(AGE_INTROD~CATEGORY, data = age_introd)
...
## test for significant diferences TIME HATCHERY ----
...
# anova
anova <- aov(TIME_HATCHERY~CATEGORY, data = time_hatch)
...
## test for significant diferences CONDITION INTRODUCED ----
...
# anova
anova <- aov(COND_INTROD~CATEGORY, data = cond_introd)
...
p5 <- ggplot(diff_succ, aes(x = CATEGORY, y = DIFF_SUCCESS, fill = CATEGORY)) +
  ...
  theme(legend.position = "none",
        axis.text.x=element_blank())
# anova
anova <- aov(DIFF_SUCCESS~CATEGORY, data = diff_succ)
...
```{r fig.cap="Figure 14: Distribution of mean age at spawning, mean at at introduction to the hatchery, and time spent in the hatchery before spawning event for breeders that participated in multiple spawning events", fig.height=4, fig.width=8}
...
```{r fig.cap="Figure 15: Reproductive success at sampling points T1 and T3 related to conditioning parameters using each spawning event as an independet observation.", fig.height=15, fig.width=10}
...
tmp <- adults %>%
  ...
  gather(key = VARIABLES, value = VALUE, 6:10) %>%
  mutate(VARIABLES = ordered(VARIABLES,
                             levels = c("AGE_INTROD", "AGE_SPAWN", "TIME_HATCHERY", "COND_INTROD", "REPROD_SUCC")))

# set formula for regression
my.formula <- y ~ x

p1 <- ggplot(tmp, aes(x = VALUE, y = SPAWN_SUCC, group = SEX, fill = SPAWNING_EVENT)) +
  ...
  facet_grid(SEX ~ VARIABLES, scales = "free") +
  scale_fill_manual(values = col_events) +
  labs(y = "reproductive success (T1)", x = "") +
  theme_facet
...
tmp <- adults %>%
  mutate(WEIGHT = ifelse(WEIGHT == 0, NA, WEIGHT),
         COND_INTROD = (WEIGHT*1000)/LENGTH,
         DIFF_SUCCESS = MEAN_SPAWN-MEAN_REPROD) %>%
  select(SAMPLE_ID, SEX, TANK, SPAWNING_EVENT, REPROD_SUCC, 
         SPAWN_SUCC, 
         COND_INTROD,
         AGE_INTROD, TIME_HATCHERY, AGE_SPAWN) %>%
  gather(key = VARIABLES, value = VALUE, 6:10) %>%
  mutate(VARIABLES = ordered(VARIABLES,
                             levels = c("AGE_INTROD", "AGE_SPAWN", "TIME_HATCHERY",
                                        "COND_INTROD", "SPAWN_SUCC")))

# set formula for regression
my.formula <- y ~ x

p2 <- ggplot(tmp, aes(x = VALUE, y = REPROD_SUCC, group = SEX, fill = SPAWNING_EVENT)) +
  geom_smooth(method = "lm", color = "black", se = FALSE, formula = my.formula) +
  geom_point(size = 2, shape = 21) +
  stat_poly_eq(formula = my.formula, 
               aes(label = paste(..rr.label.., sep = "~~~")), 
  ...
  facet_grid(SEX ~ VARIABLES, scales = "free") +
  scale_fill_manual(values = col_events) +
  labs(y = "reproductive success (T3)", x = "") +
  theme_facet
